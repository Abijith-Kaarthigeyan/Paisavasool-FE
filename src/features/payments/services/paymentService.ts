...
export type PaymentUploadListParams = {
  limit?: number
  offset?: number
  status?: string
  search?: string
...
export const paymentService = {
  uploadPaymentPdf: async (file: File): Promise<PaymentUploadCreateResponse> => {
    ...
    const response = await arApi.post<PaymentUploadCreateResponse>(
      "/payment-upload",
      formData,
      {
        headers: {
  ...
  listPaymentUploads: async (
    params?: PaymentUploadListParams
  ): Promise<PaymentUploadResponse[]> => {
    const response = await arApi.get<PaymentUploadResponse[]>("/payment-upload", {
      params,
    })
    return response.data
  },

  getFilterOptions: async (): Promise<{ uploaders: string[] }> => {
    const response = await arApi.get<{ uploaders: string[] }>(
      "/payment-upload/filter-options"
    )
    return response.data
  },
  ...
  getPaymentUploadStatus: async (id: string): Promise<PaymentUploadStatusResponse> => {
    const response = await arApi.get<PaymentUploadStatusResponse>(
      `/payment-upload/${id}/status`
    )
    return response.data
  },
}
